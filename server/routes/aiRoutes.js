...
const extractTopics = (text = "", title = "") => {
  ...
  const candidates = [
    ...
    ["Research", ["research", "study", "paper", "analysis"]],
    ["Learning", ["learn", "student", "education", "revision"]],
    ["Technology", ["software", "browser", "extension", "web", "system"]],
    ["Ethics", ["ethic", "alignment", "safety", "risk"]],
  ];
...
const localAnalysis = ({ text = "", title = "Untitled", url = "" }) => {
...
const workbookItems = async (workbook) => Research.find({ workbook: workbook || "Research Workbook" }).sort({ date: -1 });
...
router.post("/summarize", async (req, res) => {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) return res.json({ summary: localAnalysis({ text }).summary });
    ...
    res.json({ summary: localAnalysis({ text: req.body.text }).summary });
  }
});
...
router.post("/extract", async (req, res) => {
  try {
    ...
    res.json({ cleanedText, url, title, wordCount: cleanedText.split(/\s+/).length });
  } catch (error) {
...
router.post("/ask", async (req, res) => {
  try {
    ...
    const fallback = context
      ? `Based on the saved context, the strongest answer is: ${localAnalysis({ text: context }).summary}`
      : `I need extracted page or workbook context to answer: ${question}`;
    if (!process.env.GEMINI_API_KEY) return res.json({ answer: fallback });
    ...
    res.json({ answer: `I could not reach the AI model, but your saved context is available. Try again after checking the API key.` });
  }
});
...
router.post("/workbook-chat", async (req, res) => {
  try {
    ...
    const fallback = {
      introText: researchItems.length
        ? `I found ${researchItems.length} saved item${researchItems.length === 1 ? "" : "s"} in ${workbook || "Research Workbook"}.`
        : "This workbook has no saved items yet.",
...
router.get("/workbook-insights", async (req, res) => {
  try {
    ...
    const researchItems = await workbookItems(workbook);
    if (researchItems.length === 0) return res.json({ keyInsights: [], topEntities: [] });
    ...
    const fallback = {
      ...
      topEntities: Object.entries(researchItems.reduce((acc, item) => {
        [...(item.tags || []), ...(item.saveOptions || []), item.workbook].filter(Boolean).forEach((label) => { acc[label] = (acc[label] || 0) + 1; });
        return acc;
      }, {})).slice(0, 5).map(([label, count], index) => ({ label, count, color: ["text-blue-500", "text-emerald-500", "text-violet-500", "text-orange-500"][index % 4] })),
    };
    if (!process.env.GEMINI_API_KEY) return res.json(fallback);

    let contextString = `WORKBOOK CONTEXT FOR: "${workbook}"\n\n`;
    researchItems.forEach((item) => { contextString += `Topic: ${item.topic}\nSummary: ${item.summary || ""}\nNotes: ${item.notes || ""}\n\n`; });
    const model = genAI.getGenerativeModel({
      model: "gemini-2.5-flash",
...
router.post("/deep-analysis", async (req, res) => {
  try {
    const { text, title, url, wordCount } = req.body;
    if (!text) return res.status(400).json({ error: "Text content is required" });
    const fallback = localAnalysis({ text, title, url });
    if (!process.env.GEMINI_API_KEY) return res.json(fallback);
