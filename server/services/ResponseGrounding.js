class ResponseGrounding {
  ...
  async groundResponse(aiResponse, retrievedContext, query) {
    try {
      ...
      const groundedResponse = this.buildGroundedResponse(aiResponse, scoredClaims, citations, hallucinationAnalysis);
  ...
  extractClaims(content) {
    ...
    for (let i = 0; i < sentences.length; i++) {
      ...
      const claimPatterns = [/(?:according to|based on|research shows|studies indicate|evidence suggests)(.+?)/gi, /(?:it is|they are|this means|therefore|consequently)(.+?)/gi, /(?:the main reason|primary cause|key factor)(.+?)/gi, /(?:results show|findings indicate|data suggests)(.+?)/gi];
      let isClaim = false;
      ...
      if (!isClaim) {
        const factualPatterns = [/\d+(?:\.\d+)?%/, /\d+(?:\.\d+)?\s*(?:million|billion|thousand)/, /\b(?:always|never|all|none|every|only)\b/, /\b(?:discovered|invented|created|developed)\b/];
        isClaim = factualPatterns.some(pattern => pattern.test(sentence));
        if (isClaim) claimType = 'factual';
      }
      if (isClaim || sentence.length > 50) {
        claims.push({
  ...
  calculateClaimConfidence(claims, query) {
    ...
    return claims.map(claim => {
      ...
      if (claim.evidence.length > 0) {
        ...
        confidence += avgRetrievalScore / 10 * factors.retrievalScore;
      }
      if (claim.evidence.length > 0) {
  ...
  detectHallucinations(claims) {
    ...
    hallucinationAnalysis.riskScore = hallucinatedRatio * 0.5 + lowConfidenceRatio * 0.3 + contradictionRatio * 0.2;
  ...
  generateCitations(claims, context) {
    ...
    const allCitations = Array.from(citationMap.values()).sort((a, b) => b.relevance - a.relevance).slice(0, this.config.maxCitationsPerResponse);
  ...
  formatGroundedResponse(groundedResponse) {
    return {
      ...
      interactive: this.config.enableInteractiveCitations ? this.generateInteractiveElements(groundedResponse) : null
    };
  }
  ...
  assessHallucinationRisk(claim) {
    let risk = 0;
    const highRiskPatterns = [/\b(?:always|never|all|none|every|only)\b/gi, /\b(?:definitely|certainly|obviously|clearly)\b/gi, /\b(?:impossible|unbelievable|unprecedented)\b/gi, /\d{4,}/g];
  ...
  calculateContradictionScore(claim1, claim2) {
    const contradictoryPatterns = [{
      positive: /\b(?:is|are|was|were)\b/gi,
      negative: /\b(?:is not|are not|was not|were not)\b/gi
    }, {
      positive: /\b(?:always|never)\b/gi,
      negative: /\b(?:never|always)\b/gi
    }, {
      positive: /\b(?:increase|grow|rise)\b/gi,
      negative: /\b(?:decrease|fall|drop)\b/gi
    }];
...
**Recommendation:** ${this.getRiskRecommendation(analysis.overallRisk)}
  ...
  getStats() {
    return {
      config: this.config,
      trackingData: this.confidenceHistory.slice(-100),
      averageConfidence: this.confidenceHistory.length > 0 ? this.confidenceHistory.reduce((sum, entry) => sum + entry.groundingScore, 0) / this.confidenceHistory.length : 0,
      capabilities: ['source citation system', 'evidence references', 'chunk traceability', 'retrieval grounding', 'confidence indicators', 'hallucination detection', 'answer provenance', 'structured formatting', 'interactive citations']
    };
  }
