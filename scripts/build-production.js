...
class ProductionBuilder {
  ...
  async cleanBuildDirectory() {
    ...
    if (fs.existsSync(this.config.outputPath)) {
      fs.rmSync(this.config.outputPath, {
        recursive: true,
        force: true
      });
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      compiler.run((error, stats) => {
        ...
        if (stats.hasWarnings()) {
          const warningInfo = stats.toJson({
            warnings: true
          });
  ...
  async optimizeImages() {
    ...
    const images = fs.readdirSync(imageDir).filter(file => /\.(png|jpg|jpeg|gif|svg)$/i.test(file));
  ...
  async generateReports() {
    ...
    fs.writeFileSync(path.join(reportsDir, 'build-report.json'), JSON.stringify(reports, null, 2));
