...
export const useSidebarState = (options = {}) => {
  ...
  const updatePosition = useCallback((x, y) => setPosition({
    x,
    y
  }), []);
  const updateSize = useCallback((width, height) => setSize({
    width,
    height
  }), []);
...
export const useSidebarShortcuts = (shortcuts = {}) => {
  ...
  useEffect(() => {
    const handleKeyDown = event => {
      const key = [event.ctrlKey && 'ctrl', event.metaKey && 'meta', event.shiftKey && 'shift', event.altKey && 'alt', event.key.toLowerCase()].filter(Boolean).join('+');
      Object.entries(mergedShortcuts).forEach(([action, keys]) => {
        if (keys.includes(key)) {
          ...
          setTimeout(() => setActiveShortcuts(prev => ({
            ...prev,
            [action]: false
          })), 200);
        }
