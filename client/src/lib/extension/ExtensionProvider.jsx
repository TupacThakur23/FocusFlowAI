...
const initialState = {
  ...
  theme: 'dark',
  debugMode: false,
  performance: { messageCount: 0, lastActivity: null, errors: [] }
};
...
function extensionReducer(state, action) {
  switch (action.type) {
    case actionTypes.SET_CONNECTED: return { ...state, isConnected: action.payload };
    case actionTypes.SET_LOADING: return { ...state, isLoading: action.payload };
    case actionTypes.SET_ERROR: return { ...state, error: action.payload, performance: { ...state.performance, errors: [...state.performance.errors, action.payload].slice(-10) } };
    case actionTypes.CLEAR_ERROR: return { ...state, error: null };
    case actionTypes.SET_CURRENT_TAB: return { ...state, currentTab: action.payload };
    case actionTypes.SET_SELECTED_TEXT: return { ...state, selectedText: action.payload };
    case actionTypes.SET_RESEARCH_SESSION: return { ...state, researchSession: action.payload };
    case actionTypes.CLEAR_CONTENT: return { ...state, selectedText: '', researchSession: null };
    case actionTypes.SET_SIDEBAR_OPEN: return { ...state, sidebarOpen: action.payload };
    case actionTypes.SET_SIDEBAR_ANIMATING: return { ...state, sidebarAnimating: action.payload };
    case actionTypes.SET_MOBILE: return { ...state, isMobile: action.payload };
    case actionTypes.SET_ACTIVE_VIEW: return { ...state, activeView: action.payload };
    case actionTypes.SET_THEME: return { ...state, theme: action.payload };
    case actionTypes.INCREMENT_MESSAGE_COUNT: return { ...state, performance: { ...state.performance, messageCount: state.performance.messageCount + 1 } };
    case actionTypes.UPDATE_LAST_ACTIVITY: return { ...state, performance: { ...state.performance, lastActivity: action.payload } };
    case actionTypes.ADD_ERROR: return { ...state, performance: { ...state.performance, errors: [...state.performance.errors, action.payload].slice(-10) } };
    case actionTypes.CLEAR_ERRORS: return { ...state, performance: { ...state.performance, errors: [] } };
...
export const ExtensionProvider = ({ children }) => {
  ...
  const [currentTab] = useExtensionState('aideCurrentTab', { storage: 'local', defaultValue: null });
  const [selectedText] = useExtensionState('aideCurrentSelection', { storage: 'local', defaultValue: '' });

  useEffect(() => {
    const fetchActiveTab = async () => {
      try {
        if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
          chrome.runtime.sendMessage({ type: 'AIDE_GET_ACTIVE_TAB' }, (response) => {
            if (chrome.runtime.lastError) return;
            if (response && response.success && response.tab) {
              dispatch({ type: actionTypes.SET_CURRENT_TAB, payload: response.tab });
            }
          });
        }
      } catch (err) {}
  ...
  // Sync Research Session directly via chrome.storage API (no abstractions)
  useEffect(() => {
    ...
    chrome.storage.local.get(['aideResearchSession'], (data) => {
      if (data.aideResearchSession) {
        try {
          ...
          dispatch({ type: actionTypes.SET_RESEARCH_SESSION, payload: session });
        } catch (e) {}
      }
    });
    ...
    const storageListener = (changes, area) => {
      if (area === 'local' && changes.aideResearchSession) {
        try {
          ...
          if (!raw) {
            dispatch({ type: actionTypes.SET_RESEARCH_SESSION, payload: null });
            return;
          }
  ...
  useEffect(() => {
    ...
    const listener = (message) => {
      ...
      if (message.type === 'SESSION_UPDATED' && message.session) {
        dispatch({ type: actionTypes.SET_RESEARCH_SESSION, payload: message.session });
      }
    };
  ...
  useEffect(() => { dispatch({ type: actionTypes.SET_CURRENT_TAB, payload: currentTab }); }, [currentTab]);
  useEffect(() => { dispatch({ type: actionTypes.SET_SELECTED_TEXT, payload: selectedText }); }, [selectedText]);
  ...
  const actions = useMemo(() => ({
    ...
    clearContent: () => dispatch({ type: actionTypes.CLEAR_CONTENT }),
    setSidebarOpen: (open) => dispatch({ type: actionTypes.SET_SIDEBAR_OPEN, payload: open }),
    setSidebarAnimating: (animating) => dispatch({ type: actionTypes.SET_SIDEBAR_ANIMATING, payload: animating }),
    ...
    extractContent: async () => {
      try {
        if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.sendMessage) {
          return new Promise((resolve) => {
            chrome.runtime.sendMessage({ type: 'EXTRACT_CONTENT_REQUEST' }, (response) => {
    ...
    exportState: () => ({ context: state, sidebar: sidebarState.getState?.() || {}, timestamp: new Date().toISOString() })
