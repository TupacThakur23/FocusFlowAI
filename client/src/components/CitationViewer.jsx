...
import { ExternalLink, ChevronDown, ChevronRight, CheckCircle, AlertCircle, Info, Eye, EyeOff, Copy, Star, Clock, FileText, Globe, User, Shield } from 'lucide-react';
...
const CitationViewer = ({
  ...
  const renderConfidenceIndicator = () => {
    ...
    return <div className="confidence-indicator">
        ...
        <div className="confidence-bar">
          <div className={`confidence-fill ${confidenceLevel}`} style={{
          width: `${confidence * 100}%`
        }} />
        </div>

        {confidenceLevel === 'low' || confidenceLevel === 'very_low' ? <div className="confidence-warning">
  ...
  const renderTransparencyExplanation = () => {
    ...
    return <div className="transparency-explanation">
        ...
        <div className="explanation-content">
          ...
          <ul className="source-list">
            {citations.slice(0, 3).map((citation, index) => <li key={citation.id}>
                <span className="source-title">{citation.title}</span>
                {citation.relevance && <span className="source-relevance">
                    ({(citation.relevance * 100).toFixed(0)}% match)
                  </span>}
              </li>)}
            {citations.length > 3 && <li className="more-sources">
                and {citations.length - 3} more source{citations.length - 4 !== 0 ? 's' : ''}
              </li>}
          </ul>

          {evidence.length > 0 && <div className="evidence-summary">
  ...
  const renderCitation = citation => {
    ...
    return <div key={citation.id} className={`citation-item ${isExpanded ? 'expanded' : ''} ${compact ? 'compact' : ''}`}>
        <div className="citation-header" onClick={() => toggleCitation(citation.id)}>
          <div className="citation-info">
            ...
            <div className="citation-title">
              ...
              {citation.type && <span className="citation-type">{citation.type}</span>}
            </div>
          </div>

          <div className="citation-actions">
            {citation.source && <button onClick={e => {
            e.stopPropagation();
            openSource(citation.source);
          }} className="citation-action-btn" title="Open source">
          ...
          }} className={`citation-action-btn ${isCopied ? 'copied' : ''}`} title={isCopied ? 'Copied!' : 'Copy citation'}>
              {isCopied ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
            </button>

            <button className="citation-expand-btn" title={isExpanded ? 'Collapse' : 'Expand'}>
              {isExpanded ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
            </button>
          </div>
        </div>

        {isExpanded && <div className="citation-expanded">
            <div className="citation-content">
              ...
              <div className="citation-metadata">
                {citation.source && <div className="metadata-item">
                    <Globe className="w-3 h-3" />
                    <a href={citation.source} target="_blank" rel="noopener noreferrer" className="source-link">
                      {new URL(citation.source).hostname}
                    </a>
                  </div>}

                {citation.timestamp && <div className="metadata-item">
                    <Clock className="w-3 h-3" />
                    <span>{formatDate(citation.timestamp)}</span>
                  </div>}

                {citation.author && <div className="metadata-item">
                    <User className="w-3 h-3" />
                    <span>{citation.author}</span>
                  </div>}

                {citation.relevance && <div className="metadata-item">
                    <Star className="w-3 h-3" />
                    <span>Relevance: {(citation.relevance * 100).toFixed(0)}%</span>
                  </div>}
              </div>

              {citation.confidence && <div className="citation-confidence">
                  <span className="confidence-label">Source Confidence:</span>
                  <div className="confidence-mini-bar">
                    <div className="confidence-mini-fill" style={{
  ...
  const renderEvidenceSection = () => {
    ...
    return <div className="evidence-section">
        ...
        {showEvidence && <div className="evidence-list">
            {evidence.map((evidenceItem, index) => <div key={index} className="evidence-item">
                ...
                {evidenceItem.confidence && <div className="evidence-confidence">
  ...
  if (compact) {
    return <div className={`citation-viewer compact ${className}`}>
        <div className="compact-citations">
          {citations.slice(0, 3).map((citation, index) => <span key={citation.id} className="compact-citation">
              [{index + 1}]
            </span>)}
          {citations.length > 3 && <span className="compact-citation">
              +{citations.length - 3} more
            </span>}
        </div>

        {showTransparency && <div className="compact-confidence">
  ...
  return <div className={`citation-viewer ${className}`}>

      {renderConfidenceIndicator()}

      {renderTransparencyExplanation()}

      {citations.length > 0 && <div className="citations-section">
      ...
      {citations.length === 0 && showTransparency && <div className="no-sources-warning">
