...
import { BookOpen, Clock, Link2, TrendingUp, ArrowRight, Search, Filter, ChevronDown, ChevronRight, Star, Bookmark, RefreshCw, Lightbulb, GitBranch, Users, Calendar, Tag, ExternalLink } from 'lucide-react';
...
const ResearchContinuity = ({
  ...
  const [recentTopics] = useExtensionState('recentTopics', {
    storage: 'session'
  });
  const [activeWorkbooks] = useExtensionState('activeWorkbooks', {
    storage: 'session'
  });
  const [researchThreads] = useExtensionState('researchThreads', {
    storage: 'session'
  });
  const {
    addToast
  ...
  const generateContinueSuggestions = useCallback((topics, threads) => {
    ...
    if (recentTopic) {
      ...
      const isVeryRecent = timeSinceAccess < 5 * 60 * 1000;
      const isRecentlyActive = timeSinceAccess < 30 * 60 * 1000;
      if (isVeryRecent || isRecentlyActive && recentTopic.frequency > 1) {
        suggestions.push({
          ...
          description: isVeryRecent ? `Continue exploring "${recentTopic.topic}"` : `Pick up where you left off with "${recentTopic.topic}"`,
    ...
    const prioritizedThreads = activeThreads?.map(thread => ({
      ...thread,
    ...
    prioritizedThreads?.forEach(thread => {
      ...
      suggestions.push({
        ...
        description: hasRecentActivity ? `Resume active research on ${thread.topic}` : `Continue your research on ${thread.topic}`,
  ...
  const generateWorkbookSuggestions = useCallback(workbooks => {
    const suggestions = [];
    const mostAccessed = workbooks?.sort((a, b) => b.accessCount - a.accessCount)?.slice(0, 2);
  ...
  const findRelatedTopics = useCallback((primaryTopic, otherTopics) => {
    ...
    })).filter(topic => topic.similarity > 0.3).sort((a, b) => b.similarity - a.similarity);
  }, []);
  const findConceptConnections = useCallback((topics, threads) => {
    const connections = [];
    const allTopics = [...(topics || []), ...(threads?.map(t => ({
      topic: t.topic,
  ...
  const filterAndSortSuggestions = useCallback((suggestions, query, filter, sort) => {
    ...
    if (query) {
      ...
      filtered = filtered.filter(suggestion => suggestion.title.toLowerCase().includes(searchLower) || suggestion.description.toLowerCase().includes(searchLower));
    }
    if (filter !== 'all') {
  ...
  return <div className={`research-continuity ${className}`}>
      ...
      <div className="continuity-header">
        ...
        <div className="continuity-actions">
          <button onClick={refreshSuggestions} className="refresh-btn" title="Refresh suggestions">
      ...
      <div className="continuity-controls">
        <div className="search-container">
          ...
          <input type="text" placeholder="Search research suggestions..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="search-input" />
        </div>
      ...
      <div className="continuity-tabs">
        <button onClick={() => setActiveTab('suggestions')} className={`tab-btn ${activeTab === 'suggestions' ? 'active' : ''}`}>
          <Lightbulb className="w-4 h-4" />
          Suggestions
        </button>

        <button onClick={() => setActiveTab('topics')} className={`tab-btn ${activeTab === 'topics' ? 'active' : ''}`}>
          <Tag className="w-4 h-4" />
          Topics
        </button>

        <button onClick={() => setActiveTab('workbooks')} className={`tab-btn ${activeTab === 'workbooks' ? 'active' : ''}`}>
      ...
      <div className="continuity-content">
        {activeTab === 'suggestions' && <div className="suggestions-panel">

            {suggestions.some(s => s.priority === 'critical') && <div className="contextual-reminder">
                <Lightbulb className="w-4 h-4" />
                <span>Active research detected - continue your exploration</span>
              </div>}

            <div className="suggestions-list">
              {suggestions.map((suggestion, index) => <div key={suggestion.id} onClick={() => handleSuggestionClick(suggestion)} className={`suggestion-item ${selectedSuggestion?.id === suggestion.id ? 'selected' : ''} ${suggestion.priority === 'critical' ? 'critical' : ''}`}>
                  ...
                  <div className="suggestion-content">
                    <div className="suggestion-header">
                      <h3 className="suggestion-title">
                        {suggestion.title}
                        {suggestion.priority === 'critical' && <span className="live-indicator">●</span>}
                      </h3>

                      {suggestion.priority === 'critical' && <span className="critical-indicator">Active</span>}
                    </div>

                    <p className="suggestion-description">{suggestion.description}</p>

                    {suggestion.metadata && <div className="suggestion-metadata">
                        {suggestion.metadata.isVeryRecent && <div className="metadata-item recent">
                            <Clock className="w-3 h-3" />
                            Just now
                          </div>}

                        {suggestion.metadata.isUrgent && <div className="metadata-item urgent">
                            <RefreshCw className="w-3 h-3" />
                            Active
                          </div>}

                        {suggestion.metadata.hasRecentActivity && <div className="metadata-item recent">
                            <Users className="w-3 h-3" />
                            Recent activity
                          </div>}

                        {suggestion.metadata.frequency && suggestion.metadata.frequency > 1 && <div className="metadata-item">
            ...
            {suggestions.length === 0 && <div className="empty-state subtle">
        ...
        {activeTab === 'topics' && <div className="topics-panel">
            <div className="topics-grid">
              {recentTopics?.slice(0, 12).map((topic, index) => <div key={topic.topic} className="topic-card" onClick={() => handleSuggestionClick({
            id: `topic_${index}`,
            type: 'explore',
            priority: 'medium',
            title: topic.topic,
        ...
        {activeTab === 'workbooks' && <div className="workbooks-panel">
            <div className="workbooks-grid">
              {activeWorkbooks?.slice(0, 9).map((workbook, index) => <div key={workbook.id} className="workbook-card" onClick={() => handleSuggestionClick({
            id: `workbook_${index}`,
            type: 'open_workbook',
            priority: 'medium',
            title: workbook.title,
      ...
      {selectedSuggestion && <div className="suggestion-detail">
          <div className="detail-header">
            <button onClick={() => setSelectedSuggestion(null)} className="close-detail-btn">
              ×
            </button>
          ...
          <div className="detail-content">
            ...
            {selectedSuggestion.metadata && <div className="detail-metadata">
                {Object.entries(selectedSuggestion.metadata).map(([key, value]) => <div key={key} className="detail-metadata-item">
            ...
            <div className="detail-actions">
              <button onClick={() => handleSuggestionClick(selectedSuggestion)} className="primary-action-btn">
