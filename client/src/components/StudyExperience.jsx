...
const StudyExperience = ({ 
  ...
  const generateCheckpoints = useCallback(() => {
    const mockCheckpoints = sections.slice(0, 5).map((section, index) => ({
      ...
      completed: false,
      estimatedTime: Math.ceil(section.content.length / 100) // 1 minute per 100 chars
    }));
  ...
  const renderStudyControls = () => {
    return (
      <div className="study-controls">
        <button
          onClick={toggleStudy}
          className={`study-toggle ${isStudying ? 'active' : ''}`}
        >
  ...
  const renderContentSections = () => {
    return sections.map((section, index) => {
      ...
      return (
        <div
          key={section.id}
          className={`content-section ${isHighlighted ? 'highlighted' : ''} ${isFocusMode ? 'focus-mode' : ''}`}
        >
          <div className="section-header">
            ...
            <div className="section-actions">
              {hasCheckpoint && (
                <div className={`checkpoint-indicator ${completedCheckpoint ? 'completed' : ''}`}>
              ...
              <button
                ...
                title={isHighlighted ? 'Remove highlight' : 'Highlight section'}
              >
          ...
          {isHighlighted && (
            <div className="section-insights">
              ...
              <div className="insights-list">
                {insights
                  .filter(insight => insight.importance === 'high' || insight.type === 'key_concept')
                  .slice(0, 2)
                  .map(insight => (
      ...
      <div className="insights-panel">
        ...
        <div className="insights-grid">
          {insights.map(insight => (
            <div
              key={insight.id}
              className={`insight-card ${insight.importance}`}
      ...
      <div className="checkpoints-panel">
        ...
        <div className="checkpoints-list">
          {checkpoints.map((checkpoint, index) => (
            <div
              key={checkpoint.id}
              className={`checkpoint-item ${checkpoint.completed ? 'completed' : ''} ${currentCheckpoint === index ? 'current' : ''}`}
            >
              <div className="checkpoint-header">
                ...
                <div className="checkpoint-actions">
                  {checkpoint.completed ? (
                    <CheckCircle className="w-5 h-5 completed-icon" />
                  ) : (
                    <button
            ...
            <div key={index} className="concept-card">
              ...
              {concept.related && (
                <div className="related-concepts">
                  ...
                  {concept.related.slice(0, 3).map((related, i) => (
                    <span key={i} className="related-concept">{related}</span>
                  ))}
                </div>
              )}
            </div>
...
const extractKeyConcepts = (content) => {
  ...
  const conceptPatterns = [
    ...
    /\b(?:data|information|knowledge|learning)\b/gi
  ];
  ...
  return Array.from(conceptCounts.entries())
    .map(([name, frequency]) => ({
      ...
      related: [] // Would be populated with actual related concepts
    }))
