...
export default function ExtractionProgress({
  ...
  return <div className="flex flex-col items-center justify-center py-16 px-6 gap-10 animate-in fade-in zoom-in-95 duration-500">
      ...
      <div className="w-full max-w-[340px] h-[8px] bg-white/[0.03] rounded-full overflow-hidden relative border border-white/[0.05] p-[1px]">
        <div className="h-full bg-gradient-to-r from-blue-600 via-indigo-500 to-fuchsia-500 rounded-full transition-all duration-700 ease-out relative" style={{
        width: `${percent}%`
      }}>
          {}
      ...
      <div className="flex items-center justify-between w-full max-w-[340px] px-2 relative">
        ...
        return <div key={step.id} className="flex flex-col items-center gap-3">
              <div className={`w-2 h-2 rounded-full transition-all duration-500 ${isCurrent ? 'bg-white scale-150 shadow-[0_0_10px_#fff]' : isActive ? 'bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]' : 'bg-white/10'}`} />
              <span className={`text-[9px] font-black tracking-tighter transition-colors duration-500 ${isActive ? 'text-white' : 'text-gray-700'}`}>
                {step.id}
              </span>
            </div>;
      })}
