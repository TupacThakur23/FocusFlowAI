...
export default function Launcher({
      ...
      chrome.tabs.query({
        ...
        chrome.scripting.executeScript({
          ...
          func: iframeUrl => {
            ...
            panel.id = PANEL_ID;
            panel.style.cssText = ['position:fixed', 'top:0', 'right:0', `width:${PANEL_W}px`, `height:${PANEL_H}`, 'z-index:2147483647', 'display:flex', 'flex-direction:column', 'border-radius:0', 'border-left:1px solid rgba(255,255,255,0.08)', 'box-shadow:0 24px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.05)', 'background:rgba(5, 8, 22, 0.85)', 'backdrop-filter:blur(30px) saturate(150%)', 'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif', 'transition:transform 0.4s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.3s ease', 'overflow:hidden'].join(';');
            const header = document.createElement('div');
            header.style.cssText = ['height:56px', 'min-height:56px', 'background:transparent', 'display:flex', 'align-items:center', 'padding:0 20px', 'gap:12px', 'cursor:grab', 'flex-shrink:0', 'user-select:none'].join(';');
            ...
            closeBtn.onmouseenter = () => {
              closeBtn.style.background = 'rgba(220,38,38,0.2)';
            ...
            fab.id = FAB_ID;
            fab.style.cssText = ['position:fixed', 'bottom:40px', 'right:40px', 'width:64px', 'height:64px', 'border-radius:18px', 'background:#0d1117', 'border:1.5px solid rgba(100,80,255,0.3)', 'cursor:pointer', 'z-index:2147483646', 'box-shadow:0 12px 40px rgba(0,0,0,0.6), 0 0 30px rgba(99,66,255,0.25)', 'display:none', 'align-items:center', 'justify-content:center', 'transition:transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)'].join(';');
  ...
  return <div className="w-[320px] h-[440px] flex flex-col bg-[#03040b] text-white font-sans overflow-hidden">
      ...
      <div className="pt-10 pb-6 px-7">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3.5">
            <img src="icon.png" alt="FocusFlow" className="w-12 h-12 object-contain rounded-2xl" />
            <div>
      ...
      <div className="flex-1 px-6 py-2 space-y-5">
        <button onClick={handleOpenAide} className="group relative w-full p-5 bg-white/[0.03] hover:bg-white/[0.07] border border-white/[0.06] hover:border-blue-500/30 rounded-[2rem] flex items-center gap-5 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/10">
        ...
        <button onClick={handleOpenResearchHub} className="group relative w-full p-5 bg-white/[0.03] hover:bg-white/[0.07] border border-white/[0.06] hover:border-violet-500/30 rounded-[2rem] flex items-center gap-5 transition-all duration-500 hover:shadow-2xl hover:shadow-violet-500/10">
