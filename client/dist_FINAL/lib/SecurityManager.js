export class SecurityManager {
  ...
  sanitizeHTML(html) {
    ...
    if (this.securityConfig.sanitizeHTML) {
      const safeHTML = html.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '').replace(/<iframe\b[^<]*(?:(?!<\/iframe>)<[^<]*)*<\/iframe>/gi, '').replace(/<object\b[^<]*(?:(?!<\/object>)<[^<]*)*<\/object>/gi, '').replace(/<embed\b[^<]*(?:(?!<\/embed>)<[^<]*)*<\/embed>/gi, '').replace(/javascript:/gi, '').replace(/on\w+\s*=/gi, '');
  ...
  removeScripts(content) {
    ...
    return content.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '').replace(/javascript:/gi, '').replace(/on\w+\s*=/gi, '').replace(/<[^>]*on\w+\s*=\s*["'][^"']*["'][^>]*>/gi, '');
  }
  validateURL(url) {
    const result = {
      isValid: true,
  ...
  generateCSPHeader() {
    const directives = [`default-src 'self' 'nonce-${this.cspNonce}'`, `script-src 'self' 'nonce-${this.cspNonce}'`, `style-src 'self' 'nonce-${this.cspNonce}' https://fonts.googleapis.com`, `img-src 'self' data: https:`, `connect-src 'self' https://api.example.com`, `font-src 'self' https://fonts.gstatic.com`, `object-src 'none'`, `media-src 'self' https:`, `frame-src 'self'`, `child-src 'self'`, `worker-src 'self'`, `manifest-src 'self'`, `upgrade-insecure-requests`];
