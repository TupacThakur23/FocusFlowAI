export class ResearchSessionManager {
  constructor(options = {}) {
    this.config = {
      ...
      recoveryPromptDelay: options.recoveryPromptDelay || 30000,
  ...
  trackTopic(topic, metadata = {}) {
    ...
    const currentFocus = session.topicFocus.get(topic) || {
      startTime: now,
      totalTime: 0
    };
    ...
    this.logActivity('topic_tracked', {
      topic,
      metadata
    });
  ...
  trackActivity(activityType, activityData = {}) {
    ...
    this.logActivity('activity_tracked', {
      activityType,
      activityData
    });
    this.updateSessionSummary();
  ...
  trackFocusArea(focusArea, metadata = {}) {
    ...
    const currentFocus = session.focusAreaTime.get(focusArea) || {
      startTime: now,
      totalTime: 0
    };
    currentFocus.totalTime += now - (currentFocus.lastUpdate || currentFocus.startTime);
    ...
    this.logActivity('focus_tracked', {
      focusArea,
      metadata
    });
    this.persistSessions();
  ...
  getSessionSummary(sessionId = null) {
    const session = sessionId ? this.activeSessions.get(sessionId) || this.sessionHistory.find(s => s.id === sessionId) : this.currentSession;
    if (!session) return null;
    return {
      id: session.id,
  ...
  resumeSession(sessionId) {
    ...
    this.logActivity('session_resumed', {
      sessionId,
      newSessionId: resumedSession.id
    });
  ...
  getActiveTopics(limit = 10) {
    ...
    const sortedTopics = Array.from(allTopics.values()).sort((a, b) => {
  ...
  getRecentFocusAreas(limit = 5) {
    ...
    const sortedAreas = Array.from(allFocusAreas.values()).sort((a, b) => b.focusTime - a.focusTime);
  ...
  restoreWorkspaceContext(sessionId) {
    ...
    this.logActivity('workspace_restored', {
      sessionId
    });
    return context;
  }
  ...
  calculateProductivity(session) {
    ...
    const focusTime = Array.from(session.focusAreaTime?.values() || []).reduce((sum, focus) => sum + focus.totalTime, 0);
    const focusScore = Math.min(0.2, focusTime / (30 * 60 * 1000));
    score += focusScore;
  ...
  generateSessionInsights(session) {
    ...
    if (session.topicFocus && session.topicFocus.size > 0) {
      const topTopic = Array.from(session.topicFocus.entries()).sort((a, b) => b[1].totalTime - a[1].totalTime)[0];
      insights.push({
        type: 'top_topic',
        title: 'Primary Focus',
        ...
        data: {
          topic: topTopic[0],
          time: topTopic[1].totalTime
        }
      });
    }
    if (session.activities.length > 0) {
      const productiveActivities = session.activities.filter(a => ['query', 'note_created', 'synthesis'].includes(a.type));
  ...
  async loadSessions() {
    try {
      if (typeof chrome !== 'undefined' && chrome.storage) {
        ...
        if (stored) {
          const data = this.config.enableCompression ? this.decompressData(stored) : stored;
  ...
  getStats() {
    return {
      ...
      config: this.config,
      capabilities: ['session persistence', 'topic tracking', 'activity monitoring', 'focus area analysis', 'session recovery', 'workspace restoration', 'session summaries', 'productivity calculation', 'session timeline']
    };
  }
  reset() {
    if (this.currentSession) {
