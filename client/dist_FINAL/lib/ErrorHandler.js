export class ErrorHandler {
  ...
  setupGlobalHandlers() {
    ...
    if (typeof chrome !== 'undefined' && chrome.runtime) {
      chrome.runtime.onSuspend?.addListener(() => {
        this.log(null, 'Extension suspending', {
          type: 'suspend'
        });
      });
    }
  ...
  isCriticalError(errorEntry) {
    const criticalContexts = ['SidebarManager', 'StateManager'];
    const criticalMessages = ['injection failed', 'timeout', 'connection failed', 'storage quota exceeded'];
    return criticalContexts.includes(errorEntry.context) || criticalMessages.some(msg => errorEntry.message.toLowerCase().includes(msg));
  }
